...
// ─── GUILD ROLE DELETE (role deletion detection) ──────────────────────────────
client.on('roleDelete', async (role) => {
  const guild = role.guild;
  await new Promise(r => setTimeout(r, 1000)); // wait 1s for audit log to populate

  const auditLogs = await guild.fetchAuditLogs({ type: 32 /* ROLE_DELETE */, limit: 5 }).catch(() => null);
  if (!auditLogs) return;
  ...
  if (!isImmune(executor)) {
    await quarantine(executor, `Deleted role "${role.name}" (${role.id}) without being whitelisted`, guild);
  }
});
